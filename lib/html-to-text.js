...
var automate = require('./automateStates');
var automateState = automate.NORMAL;
...
function walk(dom, options, result) {
	...
	if (automateState === automate.STOPPED) {
		return result;
	}
	var whiteSpaceRegex = /\S$/;
	_.each(dom, function(elem) {
		if (options.stripper) {
			automateState = automate.getState(elem, automateState);
		}
		if (automateState === automate.STOPPED) {
			return;
		}
		switch(elem.type) {
			case 'tag':
				switch(elem.name.toLowerCase()) {
					...
					case 'div':
					case 'span':
						var spanRes = format.span(elem, walk, options);
						if (automateState != automate.STOPPED && elem.name.toLowerCase() === 'div') {
							spanRes = '<br>' + spanRes + '<br>';
						}
						result += spanRes;
						break;
					case 'p':
